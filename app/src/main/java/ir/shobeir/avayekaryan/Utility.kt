...
class Utility {
    ...
    fun nowDate(): String {
        ...
        return (calendar[Calendar.YEAR].toString() + "-" + (calendar[Calendar.MONTH] + 1) + "-"
                + calendar[Calendar.DAY_OF_MONTH] + " "
                + calendar.time.hours + ":" + calendar.time.minutes + ":"
    ...
    private inner class SolarCalendar {
        var date = 0
        var month = 0
        var year = 0
    ...
    fun day_of_week(): String {
        val MiladiDate = Date()
        MiladiDate.date = 6
        MiladiDate.month = 12
        MiladiDate.year = 1396
        val solarCalendar = SolarCalendar()
        solarCalendar.calcSolarCalendar(MiladiDate)
        return strWeekDay
    }

    fun day_of_week(year: Int, month: Int, day: Int): String {
        val now = Calendar.getInstance()
        now[year, month] = day
        val strDays = arrayOf(
            "یکشنبه", "دوشنبه", "سشنبه", "چهارشنبه", "پنجشنبه",
            "جمعه", "شنبه"
        )
        return strDays[now[Calendar.DAY_OF_WEEK]]
    }
